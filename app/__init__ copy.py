...
def create_app():
    ...
    def upload(): 
        if request.method == 'POST': 
            ...
            # # <h1>Files Uploaded Successfully.! {{ temp }}</h1>
            # # <br><p>Total: {{ len(data) }} / {{ prompt}}
            # # <br><img src='{{ image_result }}' alt='Image'></p>
            
            # # return render_template("output.html", title=temp, lend=len(data), prompt=res, image_result=image_final['data'][0]['url'])
